...
func TestPresignerMockPresignPostObject(t *testing.T) {
	t.Helper()
	presignerMock := awstoolsmock.GetPresignerMock()
	ctx := context.TODO()

	t.Run("Testing PresignPostObject", func(t *testing.T) {
		output := v4.PresignedHTTPRequest{
			URL: "test-url-123",
		}

		presignerMock.Mock.PresignPostObject.AddReturnValue(&output)
		presignerMock.Mock.PresignPostObject.AddReturnValue(&output)
		presignerMock.Mock.PresignPostObject.AddReturnValue(&output)

		for i := 0; i < 3; i++ {
			o, err := presignerMock.PresignPostObject(ctx, &s3.PutObjectInput{
				Bucket: aws.String("test-bucket"),
				Key:    aws.String(fmt.Sprintf("file-%d", i)),
			})
		...
		assert.Nil(t, o)
		assert.Equal(t, fmt.Errorf("PresignPostObject general error"), err)

		assert.Equal(t, 4, presignerMock.Mock.PresignPostObject.HasBeenCalled())
		for i := 0; i < 3; i++ {
			input := presignerMock.Mock.PresignPostObject.GetInput(i)
			assert.Equal(t, ctx, input.Ctx)
			assert.Equal(t, "test-bucket", *input.Params.Bucket)
			assert.Equal(t, fmt.Sprintf("file-%d", i), *input.Params.Key)
		}

		input := presignerMock.Mock.PresignPostObject.GetInput(3)
		assert.Equal(t, ctx, input.Ctx)
		assert.Equal(t, "test-bucket", *input.Params.Bucket)
		assert.Equal(t, "error", *input.Params.Key)
	})
